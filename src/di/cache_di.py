...
def build_cache_embedding_function() -> EmbeddingFunction:
    ...
    return EmbeddingFunction(model_id=settings.cache_embedding_model_id, device=settings.device)

@lru_cache(maxsize=1)
def build_cache_chroma_db() -> ChromaDb:
    settings = get_vector_settings()
    return ChromaDb(collection_name=settings.cache_collection_name, persist_directory=settings.cache_persist_directory, embedding_function=build_cache_embedding_function())
