...
class LocalLLmImpl(LocalLLM):
    ...
    def load(self) -> tuple:
        ...
        model = AutoModelForCausalLM.from_pretrained(self.model_id, torch_dtype=torch.float16 if self.device == 'cuda' else torch.float32).to(self.device)
        model.eval()
    ...
    def generate(self, messages: list[dict]) -> str:
        ...
        with torch.no_grad():
            outputs = self.model.generate(**inputs, max_new_tokens=self.llm_max_token, do_sample=False, repetition_penalty=1.1, pad_token_id=self.tokenizer.eos_token_id)
